...
// מחירון דמו (ש"ח)
const PRICES = {
  adanit_unit:    189,  // אדנית 60×30×30 ס"מ
  plant_small:     35,  // צמח קטן
  plant_medium:    55,  // צמח בינוני
  plant_large:     85,  // צמח גדול
  soil_liter:     1.2,  // אדמה לליטר
  perlite_liter:  2.5,  // פרלייט לליטר
  tuff_liter:     1.8,  // טוף לליטר
};

const PLANTER_VOLUME_L = 54 * 0.8; // 60×30×30 ס"מ, 80% מילוי = 43.2L
...
BALCONY DATA:
- Dimensions: ${width_m}m wide × ${depth_m}m deep (area: ${(width_m * depth_m).toFixed(1)} sqm)
- Direction: ${direction}
- Sun exposure: ${sun_pct}%
- Style preference: ${garden_style}
- Colors: floor=${floor_color}, walls=${wall_color}, railing=${railing_color}

PLANTER SPECS: rectangular 60×30×30cm, matte finish, volume=43.2L usable
...
Respond with this exact JSON structure:
{
  ...
  "perspective": {
    "vanishingPointDescription": "<where vanishing point is, e.g. upper-right third>",
  ...
  "dallePrompt": "<complete DALL-E prompt in English — detailed instructions for rendering all planters with correct perspective on this blueprint, plants fully established and lush, balcony structure remains as line drawing, only planters and plants are photorealistic and colorful>",
  "waitingFacts": [
    "<fact 1 about chosen plants, care tips, in Hebrew, 1-2 sentences>",
    "<fact 2>",
    "<fact 3>",
    "<fact 4>",
    "<fact 5>"
    ...
    L("[2] plan: " + plan.planterCount + " אדניות, " + plan.layout);

    // ── חישוב מוצרים ומחירים ──────────────────────
    ...
    // אדניות
    items.push({
      name: `אדנית מלבנית 60×30×30 ס"מ (${plan.planterColorHe})`,
    ...
    // מילוי — חישוב ממוצע על כל האדניות
    ...
    const soilBags    = Math.ceil(totalSoilL    / 20); // שק 20L
    const perliteBags = Math.ceil(totalPerliteL / 10); // שק 10L
    const tuffBags    = Math.ceil(totalTuffL    / 10); // שק 10L
    ...
    L("[3] סה"כ: ₪" + grandTotal + ", " + items.length + " פריטים");
